...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: entry ? `${entry.date.replaceAll("-", ".")} · 日常 · zhangboyang` : '日常未找到 · zhangboyang' };
}
export default async function DailyEntryPage({ params, searchParams }: Props) {
  ...
  return <main className="article-page" id="daily-content">
    ...
    <header className="article-heading"><p className="post-meta"><time dateTime={entry.date}>{entry.date.replaceAll('-', '.')}</time></p><h1 className="sr-only">{entry.date.replaceAll("-", ".")} 日常</h1><p className="article-author">zhangboyang</p></header>
    <div className="article-layout"><article className="article-body">
      <div className="article-prose">{entry.paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}<Image unoptimized src={entry.image.src} alt={entry.image.alt} width={entry.image.width} height={entry.image.height} /></div>
      <div className="article-end"><a href={backHref}>← 返回</a></div>
    </article></div>
  </main>;
}
