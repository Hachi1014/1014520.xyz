...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: post ? `${post.title} · Explore · zhangboyang` : '探索未找到 · zhangboyang' };
}
export default async function ExploreArticle({ params, searchParams }: Props) {
  ...
  return <main className="article-page" id="explore-content">
    ...
    <header className="article-heading"><p className="post-meta"><span>{post.category}</span><ThoughtDate date={post.date} endDate={post.endDate} /><span>{post.status}</span><span>约 {post.minutes} 分钟</span></p><h1>{post.title}</h1><p className="article-author">zhangboyang</p></header>
    <div className="article-layout"><article className="article-body"><div className="article-prose" dangerouslySetInnerHTML={{ __html: post.html }} /><div className="article-end"><a href={backHref}>← 返回</a>{next && <a href={articleHref('explore',next.slug,listOrigin)}>下一篇：{next.title} →</a>}</div></article>
    {post.toc.length > 0 && <aside className="article-toc"><nav aria-label="文章目录"><p>文章目录</p>{post.toc.map(heading => <a key={heading.id} href={`#${heading.id}`}>{heading.text}</a>)}</nav></aside>}</div>
  </main>;
}
