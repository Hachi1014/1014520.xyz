...
export default function ExploreListing({ embedded = false, initialView, initialCategory }: Props) {
  ...
  const rows = (items: typeof entries) => <div className="post-list">{items.map(entry => <a className="post-row thought-row" id={rowId('explore',entry.slug)} href={articleHref('explore',entry.slug,origin)} key={entry.slug}>
    ...
    <div className="post-row-copy"><p className="post-meta"><span>{entry.category}</span><ThoughtDate date={entry.date} endDate={entry.endDate} /><span>约 {entry.minutes} 分钟</span></p><ItemHeading>{entry.title}</ItemHeading></div>
    <ArrowUpRight className="post-arrow" size={24} strokeWidth={1.3} aria-hidden="true" />
  </a>)}</div>;
  return <>
    <div className="blog-index-heading"><div><p className="blog-kicker">EXPLORE / 04</p><Heading id="explore-title">探索<span>.</span></Heading></div><span className="blog-total">{entries.length} 篇探索</span></div>
    ...
    {embedded && filtered.length > visible.length && <SectionMore href={fullHref} label="查看全部探索" />}
  </>;
}
