...
export default function BlogListing({ embedded = false }: { embedded?: boolean }) {
  const visiblePosts = embedded ? [...posts].sort((a, b) => (b.publishedAt ?? '').localeCompare(a.publishedAt ?? '')).slice(0, 3) : posts;
  ...
  return <>
    ...
    <div className="post-list">{visiblePosts.map((post, i) => <a className="post-row" id={rowId("blog", post.slug)} href={articleHref("blog", post.slug, embedded ? "/" : "/blog")} key={post.slug}>
    ...
    {embedded && posts.length > visiblePosts.length && <SectionMore href="/blog" label="查看全部博客" />}
  </>;
}
